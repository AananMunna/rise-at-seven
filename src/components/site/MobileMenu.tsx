...
const sections = [
  { 
    label: "Services", 
    items: ["Search & Growth Strategy", "Onsite SEO", "Content Experience", "B2B Marketing", "Digital PR", "Social Media & Campaigns", "Data & Insights", "Social SEO/Search"] 
  },
  { label: "Industries", items: ["B2B Marketing", "Retail", "Finance", "Tech", "Travel"] },
  { label: "International", items: ["UK", "USA", "EU"] },
  {
    label: "About",
...
const links = ["Work", "Careers"];
const resourceLinks = ["Blog & Resources", "Webinar"];
...
export function MobileMenu({ open, onClose }: MobileMenuProps) {
  ...
  return (
    ...
    >
      {/* Background Overlay - এটাকে পুরো স্ক্রিনেই রাখতে পারেন অথবা শুধু প্যারেন্টের ভেতরে */}
      <div 
      ...
      <div
        className={`relative flex h-full w-full flex-col overflow-hidden rounded-[28px] bg-[#1a1a1a]/95 p-4 text-white shadow-2xl ring-1 ring-white/10 transition-transform duration-500 ${
          open ? "scale-100" : "scale-98" // একটু সুন্দর স্কেল এফেক্ট দিলাম
        }`}
      >
        {/* ... বাকি কোড সব ঠিক আছে ... */}
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <div className="w-[130px] h-auto">
            <svg viewBox="0 0 180 32" fill="currentColor" className="w-full h-full">
               ...
               <path d="M25.8409 10.7205C24.8142 10.3959 23.5183 10.0996 23.5183 8.77603C23.5183 7.77639 24.3279 7.18256 25.2728 7.18256C26.4077 7.18256 27.0549 7.91166 27.1895 8.99178H29.9984C29.9443 6.39935 27.9727 4.61719 25.4087 4.61719C22.8447 4.61719 20.7088 6.3723 20.7088 8.93767C20.7088 14.2307 27.5412 12.6102 27.5412 15.743C27.5412 17.0389 26.6227 17.7951 25.381 17.7951C23.707 17.7951 22.9516 16.6074 22.8427 15.0681H20.0352C20.1742 17.417 21.1951 20.7387 25.4087 20.7387C28.1892 20.7387 30.3494 18.5248 30.3494 15.5807C30.3494 12.6366 28.296 11.476 25.8402 10.7205H25.8409Z" />
               ...
               <path d="M9.55945 12.1512C12.1519 11.2327 13.3395 9.09953 13.3395 6.39957C13.3395 4.67151 12.7728 2.88934 11.5046 1.67395C10.0998 0.297591 8.07419 0 6.18314 0H0V19.9826H2.91572V13.8069L13.3389 19.9826V16.8606L6.22575 12.5949L7.61496 12.5293C8.26222 12.5293 8.96359 12.3676 9.55809 12.1512H9.55945ZM4.91499 10.3156H2.91572V2.67359H5.99444C8.317 2.67359 10.4231 3.86192 10.4231 6.40024C10.4231 9.5865 7.50742 10.3156 4.91499 10.3152V10.3156Z" />
          ...
          <button onClick={onClose} aria-label="Close menu" className="p-2 hover:bg-white/10 rounded-full transition-colors">
            <X className="h-6 w-6" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          <nav className="mt-6">
            {sections.map((s) => {
              ...
              return (
                <div key={s.label} className="border-b border-white/10">
                  <button
                    onClick={() => setExpanded(isOpen ? null : s.label)}
                    className="flex w-full items-center justify-between py-4"
                  >
                    <span className="font-display text-[28px] font-bold tracking-tight leading-none">
                      {s.label}
                    </span>
                  ...
                  >
                    <div className="overflow-hidden">
                      <ul className="space-y-3 pt-1 text-[18px] font-medium text-white/80">
                        {s.items.map((item) => (
                          <li key={item} className="hover:text-white cursor-pointer transition-colors px-1">
                            {item}
                          </li>
                        ))}
                      </ul>
            ...
            <ul className="mt-2">
              {links.map((l) => (
                <li key={l} className="border-b border-white/10 py-4 font-display text-[28px] font-bold tracking-tight leading-none">
                  {l}
                </li>
              ))}
              {resourceLinks.map((l) => (
                <li key={l} className="border-b border-white/10 py-4 flex items-center justify-between font-display text-[28px] font-bold tracking-tight leading-none">
                  {l}
                  <div className="grid h-6 w-6 place-items-center rounded-full bg-white/10">
                    <ChevronDown className="h-3.5 w-3.5 text-white/70" />
                  </div>
        ...
        <div className="pt-6">
          <button className="flex w-full items-center justify-center gap-2 rounded-full bg-white py-4 text-[16px] font-bold text-black transition-all hover:bg-white/90 active:scale-[0.98]">
