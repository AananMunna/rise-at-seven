...
const navLinks1 = ["Services", "Work", "About", "Culture", "Meet The Risers"];
const navLinks2 = ["Testimonials", "Blog", "Webinars", "Careers"];
const officeLinks = ["Sheffield", "Manchester", "London", "New York", "Contact"];

export function Footer() {
  return (
    <footer className="mx-3 mb-3 rounded-[28px] bg-ink px-6 py-10 text-white">
      <h3 className="text-[24px] font-semibold">Stay updated with Rise news</h3>
      <div className="mt-4 flex items-center justify-between rounded-full bg-white/10 px-5 py-3">
        <input
          placeholder="Your Email Address"
          className="flex-1 bg-transparent text-[15px] placeholder:text-white/50 outline-none"
        />
        <button className="grid h-9 w-9 place-items-center rounded-full bg-mint text-ink">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {socialLinks.map(({ icon: Icon, label }) => (
          <a
            key={label}
            href="#"
            className="flex items-center gap-1 rounded-full bg-white px-2 py-1 text-[12px] font-medium text-ink"
          >
            {typeof Icon === "string" ? (
              Icon
            ) : (
      ...
      <div className="mt-10 grid grid-cols-2 gap-6 text-[18px]">
        <ul className="space-y-3 border-l border-white/30 pl-4">
          {navLinks1.map((l) => (
            <li key={l}>{l}</li>
          ))}
        </ul>
      ...
      <ul className="mt-8 space-y-3 border-l border-white/30 pl-4 text-[18px]">
        {officeLinks.map((l) => (
          <li key={l}>{l}</li>
        ))}
      </ul>
       ...
       <a
        ...
        >
          <div className="w-[400px] mt-15 h-auto">
             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 180 32" fill="currentColor" className="w-full h-full">
      ...
      <div className="mt-8 space-y-1 text-[12px] text-white/70">
